...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_MEASURED_LIST_DERIVED_DATA:-"$IOS_DIR/build/device-measured-list-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_MEASURED_LIST_BUNDLE_ID:-dev.solidnative.measuredlist}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/list-measured.ios.bundle.map"
RESULTS_DIRECTORY="$DERIVED_DATA_PATH/Logs/Test"
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeMeasuredList-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=list-measured.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -resultBundlePath "$RESULT_BUNDLE_PATH" \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testMeasuredVirtualizedListOnPhysicalDevice \
  test
XCODEBUILD_STATUS=$?
set -e
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-measured-list-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_MEASURED_VIRTUALIZED_LIST_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS measured-list proof emitted its JavaScript failure marker." >&2
  exit 1
fi
...
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS measured-list physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified physical iPhone intrinsic row geometry, bounded measured windowing, keyed cleanup, and native scroll eviction on $SOLID_NATIVE_IOS_DESTINATION."
