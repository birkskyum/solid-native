...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_PRESSABLE_DERIVED_DATA:-"$IOS_DIR/build/device-pressable-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_PRESSABLE_BUNDLE_ID:-dev.solidnative.e2e}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/pressable.ios.bundle.map"
...
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativePressable-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=pressable.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testPressableOnPhysicalDevice \
  clean build-for-testing

node "$APP_DIR/scripts/verify-solid-runtime-sourcemap.mjs" "$SOURCE_MAP_PATH"
node "$APP_DIR/scripts/verify-pressable-sourcemap.mjs" "$SOURCE_MAP_PATH"

if [ ! -d "$APP_PATH" ] || [ ! -d "$TEST_BUNDLE_PATH" ]; then
  echo "The signed iOS Pressable app or UI-test bundle is missing after build-for-testing." >&2
  exit 1
fi
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-pressable-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_PRESSABLE_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS Pressable proof emitted its JavaScript failure marker." >&2
  exit 1
fi
...
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS Pressable physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified physical iPhone Pressable diagonal exit/recovery, displaced-release recovery, ScrollView takeover, teardown, and cleanup on $SOLID_NATIVE_IOS_DESTINATION."
