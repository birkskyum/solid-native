...
test_class=dev.solidnative.e2e.SolidNativeSafeAreaPhysicalTest#testNativeSafeAreaDeliveryAndLayoutOnPhysicalDevice
...
solid_native_android_prepare_device "$serial" "physical safe-area proof"
pnpm --dir "$repo_root" --filter '@solid-native/native-e2e...' build
ENTRY_FILE=safe-area.tsx sh "$script_dir/android-gradle.sh" \
...
case "$result" in
  ...
  *)
    echo "The Android safe-area instrumentation did not pass exactly one test." >&2
    exit 1
    ;;
esac
...
do
  ...
  if printf '%s\n' "$logs" | grep -F "SOLID_NATIVE_SAFE_AREA_FAILED" >/dev/null; then
    echo "The Android safe-area proof emitted its JavaScript failure marker." >&2
    exit 1
  fi
  missing_marker=
  ...
  if [ "$(date +%s)" -ge "$marker_deadline" ]; then
    echo "The Android safe-area proof omitted marker $missing_marker after its teardown deadline." >&2
    exit 1
  fi
  sleep 0.1
done

echo "Verified native safe-area delivery, physical system inset layout, Solid content gating, and non-terminating teardown on $serial."
