...
test_class=dev.solidnative.e2e.SolidNativeSharingPhysicalTest#testNativeSharesheetPayloadsAndTeardownOnPhysicalDevice
...
solid_native_android_prepare_device "$serial" "physical sharing proof"
pnpm --dir "$repo_root" --filter '@solid-native/native-e2e...' build
ENTRY_FILE=sharing.tsx sh "$script_dir/android-gradle.sh" \
...
case "$result" in
  ...
  *)
    echo "The Android sharing instrumentation did not pass exactly one test." >&2
    exit 1
    ;;
esac
...
do
  ...
  if printf '%s\n' "$logs" | grep -F "SOLID_NATIVE_SHARING_FAILED" >/dev/null; then
    echo "The Android sharing proof emitted its JavaScript failure marker." >&2
    exit 1
  fi
  missing_marker=
  ...
  if [ "$(date +%s)" -ge "$marker_deadline" ]; then
    echo "The Android sharing proof omitted marker $missing_marker." >&2
    exit 1
  fi
  sleep 1
done
...
do
  ...
  if [ "$uninstall_result" != "Success" ]; then
    echo "Android sharing proof could not remove $package_name: $uninstall_result" >&2
    exit 1
  fi
done
for package_name in "$test_id" "$app_id"
do
  if adb -s "$serial" shell pidof "$package_name" | grep -E '[0-9]' >/dev/null; then
    echo "Android sharing process $package_name survived verified cleanup." >&2
    exit 1
  fi
  if adb -s "$serial" shell pm path "$package_name" 2>/dev/null | grep -F 'package:' >/dev/null; then
    echo "Android sharing package $package_name survived verified cleanup." >&2
    exit 1
  fi
done

echo "Verified native Android sharesheet presentation, OS-boundary payload mapping, causal commits, and non-terminating teardown on $serial."
