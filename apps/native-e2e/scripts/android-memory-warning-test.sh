...
test_class=dev.solidnative.e2e.SolidNativeMemoryWarningPhysicalTest#testMemoryWarningOnPhysicalDevice
...
solid_native_android_prepare_device "$serial" "physical memory-warning proof"
pnpm --dir "$repo_root" --filter '@solid-native/native-e2e...' build
ENTRY_FILE=memory-warning.tsx sh "$script_dir/android-gradle.sh" \
...
case "$result" in
  ...
  *)
    echo "The Android memory-warning instrumentation did not pass exactly one test." >&2
    exit 1
    ;;
esac
...
while :
do
  logs=$(adb -s "$serial" logcat -d -v threadtime ReactNativeJS:I SolidNativeE2E:I '*:S')
  if printf '%s\n' "$logs" | grep -F "SOLID_NATIVE_MEMORY_WARNING_FAILED" >/dev/null; then
    echo "The Android memory-warning proof emitted its JavaScript failure marker." >&2
    exit 1
  fi
  missing_marker=
  for marker in \
    "SOLID_NATIVE_MEMORY_WARNING_READY" \
    "SOLID_NATIVE_MEMORY_WARNING_CAUSALITY_SUCCEEDED" \
    "SOLID_NATIVE_MEMORY_WARNING_TEARDOWN_REQUESTED" \
    "SOLID_NATIVE_SURFACE_STATUS surface-stopped" \
    "SOLID_NATIVE_MEMORY_WARNING_TEARDOWN_SUCCEEDED"
  do
  ...
  if [ "$(date +%s)" -ge "$marker_deadline" ]; then
    echo "The Android memory-warning proof omitted marker $missing_marker." >&2
    exit 1
  fi
  sleep 1
done
...
printf '%s\n' "$logs" |
  grep -E 'SOLID_NATIVE_MEMORY_WARNING_|SOLID_NATIVE_SURFACE_STATUS surface-stopped'
echo "Verified Android Application memory pressure -> SolidNativePlatformAndroid -> Hermes -> Solid computation -> Fabric commit -> mount -> frame -> owner teardown on $serial."
