...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_STATUS_BAR_DERIVED_DATA:-"$IOS_DIR/build/device-status-bar-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_STATUS_BAR_BUNDLE_ID:-dev.solidnative.statusbar}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/status-bar.ios.bundle.map"
...
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeStatusBar-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=status-bar.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testStatusBarOwnerStackOnPhysicalDevice \
  clean build-for-testing

node "$APP_DIR/scripts/verify-solid-runtime-sourcemap.mjs" "$SOURCE_MAP_PATH"

if [ ! -d "$APP_PATH" ] || [ ! -d "$TEST_BUNDLE_PATH" ]; then
  echo "The signed iOS status-bar app or UI-test bundle is missing after build-for-testing." >&2
  exit 1
fi
...
if [ "$(/usr/libexec/PlistBuddy -c 'Print :UIViewControllerBasedStatusBarAppearance' "$APP_PATH/Info.plist")" != "false" ]; then
  echo "The signed iOS status-bar app did not disable view-controller-based ownership." >&2
  exit 1
fi
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-status-bar-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_STATUS_BAR_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS status-bar proof emitted its JavaScript failure marker." >&2
  exit 1
fi
...
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS status-bar physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified physical iPhone status-bar contrast, visibility, Solid owner restoration, causal commits, and non-terminating teardown on $SOLID_NATIVE_IOS_DESTINATION."
