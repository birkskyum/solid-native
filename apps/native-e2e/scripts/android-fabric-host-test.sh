...
test_class=dev.solidnative.e2e.FabricHostPhysicalTest#testFrameworkFreeFabricHostOnPhysicalDevice
...
finish_cleanup() {
  ...
  if adb -s "$serial" shell pidof "$app_id" >/dev/null 2>&1; then
    echo "The framework-free Fabric Host app remained alive after force-stop." >&2
    exit 1
  fi
  ...
  if [ -n "$(adb -s "$serial" shell pm path "$test_id" 2>/dev/null)" ] || \
    ...
    echo "A disposable framework-free Fabric Host package remained installed." >&2
    exit 1
  fi
}

solid_native_android_lease_stay_awake "$serial"
solid_native_android_prepare_device "$serial" "physical Fabric Host proof"
ENTRY_FILE=fabric-host.ts sh "$script_dir/android-gradle.sh" \
...
case "$result" in
  ...
  *)
    echo "The framework-free Fabric Host instrumentation did not pass exactly one test." >&2
    exit 1
    ;;
esac
...
while :; do
  ...
  if printf '%s\n' "$logs" | grep -F "SOLID_NATIVE_FABRIC_HOST_FAILED" >/dev/null; then
    echo "The framework-free Fabric Host emitted a JavaScript failure marker." >&2
    exit 1
  fi
  missing_marker=
  for marker in \
  ...
  if [ "$marker_attempt" -ge 150 ]; then
    echo "Missing framework-free Fabric Host marker after teardown wait: $missing_marker" >&2
    exit 1
  fi
  sleep 0.1
done
...
pid=$(printf '%s\n' "$logs" | awk \
  '/SOLID_NATIVE_FABRIC_HOST_READY/ { print $3; exit }')
case "$pid" in
  ''|*[!0-9]*)
    echo "The framework-free Fabric Host ready marker did not expose a valid process ID." >&2
    exit 1
    ;;
esac
finish_cleanup
echo "Verified framework-free Fabric Host mount, physical event, native update, lifecycle, and exact teardown on $serial (process $pid)."
