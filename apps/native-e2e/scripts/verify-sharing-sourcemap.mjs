...
const REQUIRED_SOURCE_PATTERNS = Object.freeze([
  /\/apps\/native-e2e\/sharing\.tsx$/u,
  /\/packages\/sharing\/dist\/index\.js$/u,
  /\/packages\/sharing\/dist\/react-native-adapter\.js$/u,
  /\/packages\/sharing\/dist\/react-native\.js$/u,
  /\/packages\/sharing\/dist\/solid\.js$/u,
]);
...
export function assertSharingBundle(map) {
  ...
  for (const pattern of REQUIRED_SOURCE_PATTERNS) {
    ...
    if (matches.length !== 1) {
      throw new Error(
        `Expected exactly one sharing proof source ${String(pattern)}, found ${JSON.stringify(matches)}.`,
      );
    }
  ...
  const unexpectedPackageSources = sources.filter(
    (source) =>
      source.includes("/packages/sharing/") && !matchedSources.includes(source),
  );
  if (unexpectedPackageSources.length > 0) {
    throw new Error(
      `The sharing proof retained unexpected package sources: ${JSON.stringify(unexpectedPackageSources)}.`,
    );
  }
  ...
  if (forbiddenFacades.length > 0) {
    throw new Error(
      `The sharing proof retained React Native sharing facades: ${JSON.stringify(forbiddenFacades)}.`,
    );
  }
...
function main() {
  ...
  const evidence = assertSharingBundle(map);
  console.log(
    `Verified the complete direct-native sharing seam in ${absolutePath}: ${JSON.stringify(evidence)}.`,
  );
}
