...
if [ "$(adb -s "$serial" shell getprop ro.kernel.qemu | tr -d '\r')" = "1" ]; then
  echo "The Linking handoff proof requires physical Android hardware." >&2
  exit 1
fi
...
settings_id=com.android.settings
runner="$test_id/androidx.test.runner.AndroidJUnitRunner"
test_class=dev.solidnative.e2e.SolidNativeLinkingPhysicalTest#testOutboundApplicationURLAndSettingsHandoffsOnPhysicalDevice

leave_settings_if_focused() {
  if adb -s "$serial" shell dumpsys window | grep -E "mCurrentFocus=.*$settings_id" >/dev/null; then
...
solid_native_android_prepare_device "$serial" "physical Linking proof"
pnpm --dir "$repo_root" --filter '@solid-native/native-e2e...' build
ENTRY_FILE=linking.tsx sh "$script_dir/android-gradle.sh" \
...
case "$result" in
  ...
  *)
    echo "The Android Linking instrumentation did not pass exactly one test." >&2
    exit 1
    ;;
esac

logs=$(adb -s "$serial" logcat -d -v threadtime ReactNativeJS:I '*:S')
if printf '%s\n' "$logs" | grep -F "SOLID_NATIVE_LINKING_FAILED" >/dev/null; then
  echo "The Android Linking proof emitted its JavaScript failure marker." >&2
  exit 1
fi
for marker in \
...
do
  marker_count=$(printf '%s\n' "$logs" | grep -c -F "$marker")
  if [ "$marker_count" -ne 1 ]; then
    echo "The Android Linking proof produced $marker_count occurrences of $marker instead of exactly one." >&2
    exit 1
  fi
done
...
do
  ...
  if [ "$uninstall_result" != "Success" ]; then
    echo "Android Linking proof could not remove $package_name: $uninstall_result" >&2
    exit 1
  fi
done
for package_name in "$test_id" "$app_id"
do
  if adb -s "$serial" shell pidof "$package_name" | grep -E '[0-9]' >/dev/null; then
    echo "Android Linking process $package_name survived verified cleanup." >&2
    exit 1
  fi
  if adb -s "$serial" shell pm path "$package_name" 2>/dev/null | grep -F 'package:' >/dev/null; then
    echo "Android Linking package $package_name survived verified cleanup." >&2
    exit 1
  fi
done

echo "Verified direct Linking capability, self-URL OS delivery, private causal Fabric commit, Settings handoff, return, and teardown on $serial."
