...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_SAFE_AREA_DERIVED_DATA:-"$IOS_DIR/build/device-safe-area-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_SAFE_AREA_BUNDLE_ID:-dev.solidnative.safearea}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/safe-area.ios.bundle.map"
...
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeSafeArea-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=safe-area.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testSafeAreaDeliveryAndRotationOnPhysicalDevice \
  clean build-for-testing

node "$APP_DIR/scripts/verify-solid-runtime-sourcemap.mjs" "$SOURCE_MAP_PATH"

if [ ! -d "$APP_PATH" ] || [ ! -d "$TEST_BUNDLE_PATH" ]; then
  echo "The signed iOS safe-area app or UI-test bundle is missing after build-for-testing." >&2
  exit 1
fi
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-safe-area-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_SAFE_AREA_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS safe-area proof emitted its JavaScript failure marker." >&2
  exit 1
fi

for marker in \
  SOLID_NATIVE_SAFE_AREA_PROVIDER_MOUNTED \
  SOLID_NATIVE_SAFE_AREA_METRICS_OBSERVED \
  SOLID_NATIVE_SAFE_AREA_CONTENT_MOUNTED \
  SOLID_NATIVE_SAFE_AREA_CAUSALITY_SUCCEEDED \
  SOLID_NATIVE_SAFE_AREA_TEARDOWN_SUCCEEDED
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS safe-area physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified physical iPhone notch/home-indicator insets, rotated safe-area delivery, causal Fabric commits, and non-terminating teardown on $SOLID_NATIVE_IOS_DESTINATION."
