...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_NAVIGATION_SHEET_DERIVED_DATA:-"$IOS_DIR/build/device-navigation-sheet-tests"}
...
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeNavigationSheet-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testNavigationSheetDetentsOnPhysicalDevice \
  test
XCODEBUILD_STATUS=$?
set -e
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-navigation-sheet-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_NAVIGATION_PROCESS_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS native-sheet proof emitted its JavaScript failure marker." >&2
  exit 1
fi
...
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS native-sheet proof marker: $marker" >&2
    exit 1
  fi
done
...
if [ "$PROCESS_COUNT" -ne 1 ]; then
  echo "Expected exactly one iOS native-sheet app process; found $PROCESS_COUNT." >&2
  exit 1
fi

echo "Verified Solid-owned iPhone native-sheet presentation, physical detent change, event delivery, and teardown on $SOLID_NATIVE_IOS_DESTINATION."
