...
apk="$android_dir/app/build/outputs/apk/reactControl/release/app-reactControl-release.apk"

ENTRY_FILE=control.ts SOLID_NATIVE_REACT_CONTROL=1 \
  sh "$script_dir/android-gradle.sh" :app:assembleReactControlRelease
adb -s "$serial" install -r "$apk"
adb -s "$serial" shell am start -S -W \
  -n dev.solidnative.control/dev.solidnative.e2e.MainActivity
