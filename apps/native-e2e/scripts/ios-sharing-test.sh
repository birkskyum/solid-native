...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_SHARING_DERIVED_DATA:-"$IOS_DIR/build/device-sharing-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_SHARING_BUNDLE_ID:-dev.solidnative.sharing}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/sharing.ios.bundle.map"
...
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeSharing-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=sharing.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testNativeShareSheetCompletionDismissalAndTeardownOnPhysicalDevice \
  clean build-for-testing

node "$APP_DIR/scripts/verify-solid-runtime-sourcemap.mjs" "$SOURCE_MAP_PATH"
node "$APP_DIR/scripts/verify-sharing-sourcemap.mjs" "$SOURCE_MAP_PATH"

if [ ! -d "$APP_PATH" ] || [ ! -d "$TEST_BUNDLE_PATH" ]; then
  echo "The signed iOS sharing app or UI-test bundle is missing after build-for-testing." >&2
  exit 1
fi
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-sharing-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_SHARING_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS sharing proof emitted its JavaScript failure marker." >&2
  exit 1
fi

for marker in \
  SOLID_NATIVE_SHARING_READY \
  SOLID_NATIVE_SHARING_COMBINED_CAUSALITY_SUCCEEDED \
  SOLID_NATIVE_SHARING_URL_ONLY_CAUSALITY_SUCCEEDED \
  SOLID_NATIVE_SHARING_TEARDOWN_SUCCEEDED
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS sharing physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified native iPhone activity-sheet completion, dismissal, private causal Fabric commits, and non-terminating teardown on $SOLID_NATIVE_IOS_DESTINATION."
