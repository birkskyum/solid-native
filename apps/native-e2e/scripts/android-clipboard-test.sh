...
test_class=dev.solidnative.e2e.SolidNativeClipboardPhysicalTest#testNativeClipboardBoundaryCausalityAndTeardownOnPhysicalDevice
...
solid_native_android_prepare_device "$serial" "physical clipboard proof"
pnpm --dir "$repo_root" --filter '@solid-native/native-e2e...' build
ENTRY_FILE=clipboard.tsx sh "$script_dir/android-gradle.sh" \
...
case "$result" in
  ...
  *)
    echo "The Android clipboard instrumentation did not pass exactly one test." >&2
    exit 1
    ;;
esac
...
do
  ...
  if printf '%s\n' "$logs" | grep -F "SOLID_NATIVE_CLIPBOARD_FAILED" >/dev/null; then
    echo "The Android clipboard proof emitted its JavaScript failure marker." >&2
    exit 1
  fi
  missing_marker=
  ...
  if [ "$(date +%s)" -ge "$marker_deadline" ]; then
    echo "The Android clipboard proof omitted marker $missing_marker." >&2
    exit 1
  fi
  sleep 1
done
...
do
  ...
  if [ "$uninstall_result" != "Success" ]; then
    echo "Android clipboard proof could not remove $package_name: $uninstall_result" >&2
    exit 1
  fi
done
for package_name in "$test_id" "$app_id"
do
  if adb -s "$serial" shell pidof "$package_name" | grep -E '[0-9]' >/dev/null; then
    echo "Android clipboard process $package_name survived verified cleanup." >&2
    exit 1
  fi
  if adb -s "$serial" shell pm path "$package_name" 2>/dev/null | grep -F 'package:' >/dev/null; then
    echo "Android clipboard package $package_name survived verified cleanup." >&2
    exit 1
  fi
done

echo "Verified independent Android clipboard write/read/clear boundaries, private causal commits, original-value restoration, and non-terminating teardown on $serial."
