...
const TEST_CLASS =
  "dev.solidnative.e2e.SolidNativeNetworkingPhysicalTest#testNativeNetworkingProtocolsAndOwnerTeardownOnPhysicalDevice";
const TEST_RUNNER = `${TEST_ID}/androidx.test.runner.AndroidJUnitRunner`;
const DEVICE_PORT = 38_473;
...
async function waitForMarkers(serial) {
  ...
  while (performance.now() < deadline) {
    ...
    if (logs.includes("SOLID_NATIVE_NETWORKING_FAILED")) {
      throw new Error(`The physical networking app reported failure:\n${logs}`);
    }
  ...
  throw new Error(
    `The physical networking app omitted markers ${JSON.stringify(missing)}:\n${logs}`,
  );
}
...
async function main() {
  ...
  if (adb("shell", "getprop", "ro.kernel.qemu") === "1") {
    throw new Error("The networking proof requires physical Android hardware.");
  }
  ...
  runStreaming(
    ...
    {
      cwd: appDirectory,
      env: { ...process.env, ENTRY_FILE: "networking.tsx" },
    },
  );
  ...
  run(process.execPath, [
    path.join(scriptDirectory, "verify-networking-sourcemap.mjs"),
    sourceMap,
  ]);
  ...
  const cleanup = async (failClosed) => {
    ...
    if (remainingPid.status === 0 && remainingPid.stdout.trim() !== "") {
      failures.push(
        new Error(`Networking process ${APP_ID} survived cleanup.`),
      );
    }
    for (const packageName of [TEST_ID, APP_ID]) {
      ...
      if (installed.status === 0 && installed.stdout.trim() !== "") {
        failures.push(
          new Error(`Networking package ${packageName} survived cleanup.`),
        );
      }
    }
    ...
    if (
      ...
      failures.push(
        new Error("The networking reverse tunnel survived cleanup."),
      );
    }
    try {
    ...
    if (failClosed && failures.length > 1) {
      throw new AggregateError(failures, "Networking proof cleanup failed.");
    }
  };
  ...
  try {
    // Installation is inside the cleanup boundary so a partial app/test APK
    // install cannot survive a later install or tunnel failure.
    ...
    if (
      ...
      throw new Error(
        "adb did not retain the networking proof reverse tunnel.",
      );
    }
    adb("logcat", "-c");
    ...
    if (
      ...
      throw new Error(
        `Networking instrumentation did not report one passing test:\n${instrumentation}`,
      );
    }
    ...
    const readyLine = logs
      .split(/\r?\n/u)
      .find((line) => line.includes("SOLID_NATIVE_NETWORKING_READY"));
    const pid = readyLine?.trim().split(/\s+/u)[2];
    if (pid === undefined || !/^\d+$/u.test(pid)) {
      throw new Error("The networking ready marker omitted its process ID.");
    }
    process.stdout.write(
      `Verified physical Android native networking on ${serial} (process ${pid}): ${JSON.stringify(evidence)}.\n`,
    );
  } finally {
