...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_SECURE_STORAGE_DERIVED_DATA:-"$IOS_DIR/build/device-secure-storage-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_SECURE_STORAGE_BUNDLE_ID:-dev.solidnative.securestorage}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/secure-storage.ios.bundle.map"
RESULTS_DIRECTORY="$DERIVED_DATA_PATH/Logs/Test"
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeSecureStorage-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=secure-storage.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -resultBundlePath "$RESULT_BUNDLE_PATH" \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testSecureStorageOnPhysicalDevice \
  test
XCODEBUILD_STATUS=$?
set -e
...
node "$APP_DIR/scripts/verify-secure-storage-sourcemap.mjs" "$SOURCE_MAP_PATH"

DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-secure-storage-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_SECURE_STORAGE_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS secure-storage proof emitted its JavaScript failure marker." >&2
  exit 1
fi

for marker in \
  SOLID_NATIVE_SECURE_STORAGE_READY \
...
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS secure-storage physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified iOS Keychain process persistence, exact private restore, deletion, causal privacy, and non-terminating teardown on $SOLID_NATIVE_IOS_DESTINATION."
