...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_DEVTOOLS_DERIVED_DATA:-"$IOS_DIR/build/device-devtools-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_DEVTOOLS_BUNDLE_ID:-dev.solidnative.e2e}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/devtools.ios.bundle.map"
RESULTS_DIRECTORY="$DERIVED_DATA_PATH/Logs/Test"
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeDevtools-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=devtools.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -resultBundlePath "$RESULT_BUNDLE_PATH" \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testDevelopmentOverlayOnPhysicalDevice \
  test
XCODEBUILD_STATUS=$?
set -e
...
node "$APP_DIR/scripts/verify-devtools-sourcemap.mjs" "$SOURCE_MAP_PATH"

DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-devtools-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_DEVTOOLS_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS development-overlay proof emitted its JavaScript failure marker." >&2
  exit 1
fi
...
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS development-overlay physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified repeated network and causal panel ownership, on-device Solid/native correlation, all development error recovery paths, bridge restoration, and non-terminating teardown on $SOLID_NATIVE_IOS_DESTINATION."
