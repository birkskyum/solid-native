...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_PLATFORM_REACTIVITY_DERIVED_DATA:-"$IOS_DIR/build/device-platform-reactivity-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_PLATFORM_REACTIVITY_BUNDLE_ID:-dev.solidnative.platformreactivity}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/platform-reactivity.ios.bundle.map"
...
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativePlatformReactivity-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=platform-reactivity.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testPlatformReactivityOnPhysicalDevice \
  clean build-for-testing

node "$APP_DIR/scripts/verify-solid-runtime-sourcemap.mjs" "$SOURCE_MAP_PATH"
node "$APP_DIR/scripts/verify-platform-reactivity-sourcemap.mjs" "$SOURCE_MAP_PATH"

if [ ! -d "$APP_PATH" ] || [ ! -d "$TEST_BUNDLE_PATH" ]; then
  echo "The signed iOS platform-reactivity app or UI-test bundle is missing after build-for-testing." >&2
  exit 1
fi
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-platform-reactivity-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_PLATFORM_REACTIVITY_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS platform-reactivity proof emitted its JavaScript failure marker." >&2
  exit 1
fi
...
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS platform-reactivity physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified physical iPhone rotation and system-appearance delivery, causal Fabric commits, preference restoration, and non-terminating teardown on $SOLID_NATIVE_IOS_DESTINATION."
