...
test_class=dev.solidnative.e2e.SolidNativeRefreshableScrollViewPhysicalTest#testControlledPullToRefreshOnPhysicalDevice
...
solid_native_android_prepare_device "$serial" "physical pull-to-refresh proof"
pnpm --dir "$repo_root" --filter '@solid-native/native-e2e...' build
ENTRY_FILE=refreshable-scroll-view.tsx sh "$script_dir/android-gradle.sh" \
...
case "$result" in
  ...
  *)
    echo "The Android pull-to-refresh instrumentation did not pass exactly one test." >&2
    exit 1
    ;;
esac
...
do
  ...
  if printf '%s\n' "$logs" | grep -F "SOLID_NATIVE_REFRESHABLE_SCROLL_FAILED" >/dev/null; then
    echo "The Android pull-to-refresh proof emitted its JavaScript failure marker." >&2
    exit 1
  fi
  missing_marker=
  ...
  if [ "$(date +%s)" -ge "$marker_deadline" ]; then
    echo "The Android pull-to-refresh proof omitted marker $missing_marker after teardown." >&2
    exit 1
  fi
  sleep 0.1
done

echo "Verified physical Android pull-to-refresh rejection, controlled acceptance/completion, stable native identity, and teardown on $serial."
