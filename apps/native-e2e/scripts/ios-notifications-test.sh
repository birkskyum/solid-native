...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_NOTIFICATIONS_DERIVED_DATA:-"$IOS_DIR/build/device-notification-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_NOTIFICATIONS_BUNDLE_ID:-dev.solidnative.notification}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/notifications.ios.bundle.map"
...
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeNotifications-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
INSTALL_ATTEMPTED=0
...
xcodebuild \
  ...
  ENTRY_FILE=notifications.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testNativeNotificationDeliveryCancellationAndTeardownOnPhysicalDevice \
  clean build-for-testing

node "$APP_DIR/scripts/verify-solid-runtime-sourcemap.mjs" "$SOURCE_MAP_PATH"
node "$APP_DIR/scripts/verify-notifications-sourcemap.mjs" "$SOURCE_MAP_PATH"

if [ ! -d "$APP_PATH" ] || [ ! -d "$TEST_BUNDLE_PATH" ]; then
  echo "The signed iOS notification app or UI-test bundle is missing after build-for-testing." >&2
  exit 1
fi
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-notifications-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_NOTIFICATION_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS notification proof emitted its JavaScript failure marker." >&2
  exit 1
fi

for marker in \
  SOLID_NATIVE_NOTIFICATION_READY \
  ...
  SOLID_NATIVE_NOTIFICATION_TEARDOWN_SUCCEEDED
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS notification physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified native iPhone wrapper-free notification delivery, private causal Fabric commit, cancellation, and non-terminating teardown on $SOLID_NATIVE_IOS_DESTINATION."
