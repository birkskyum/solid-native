...
IOS_DIR="$APP_DIR/ios"
DERIVED_DATA_PATH=${SOLID_NATIVE_IOS_LINKING_DERIVED_DATA:-"$IOS_DIR/build/device-linking-tests"}
BUNDLE_IDENTIFIER=${SOLID_NATIVE_IOS_LINKING_BUNDLE_ID:-dev.solidnative.linking}
SOURCE_MAP_PATH="$DERIVED_DATA_PATH/linking.ios.bundle.map"
...
RESULT_BUNDLE_PATH="$RESULTS_DIRECTORY/SolidNativeLinking-$(date +%Y%m%d-%H%M%S).xcresult"
DIAGNOSTICS_PARENT=
...
xcodebuild \
  ...
  ENTRY_FILE=linking.tsx \
  SOLID_NATIVE_APP_BUNDLE_ID="$BUNDLE_IDENTIFIER" \
  SOURCEMAP_FILE="$SOURCE_MAP_PATH" \
  -allowProvisioningUpdates \
  -collect-test-diagnostics never \
  -only-testing:SolidNativeE2EUITests/SolidNativeE2EUITests/testOutboundApplicationURLAndSettingsHandoffsOnPhysicalDevice \
  clean build-for-testing

node "$APP_DIR/scripts/verify-solid-runtime-sourcemap.mjs" "$SOURCE_MAP_PATH"
node "$APP_DIR/scripts/verify-linking-sourcemap.mjs" "$SOURCE_MAP_PATH"

if [ ! -d "$APP_PATH" ] || [ ! -d "$TEST_BUNDLE_PATH" ]; then
  echo "The signed iOS Linking app or UI-test bundle is missing after build-for-testing." >&2
  exit 1
fi
...
if [ "$ACTUAL_URL_SCHEME" != "$BUNDLE_IDENTIFIER" ]; then
  echo "Expected registered iOS URL scheme $BUNDLE_IDENTIFIER; found $ACTUAL_URL_SCHEME." >&2
  exit 1
fi
ACTUAL_QUERY_SCHEME=$(
  /usr/libexec/PlistBuddy -c 'Print :LSApplicationQueriesSchemes:0' "$APP_PATH/Info.plist"
)
if [ "$ACTUAL_QUERY_SCHEME" != "$BUNDLE_IDENTIFIER" ]; then
  echo "Expected queryable iOS URL scheme $BUNDLE_IDENTIFIER; found $ACTUAL_QUERY_SCHEME." >&2
  exit 1
fi
...
DIAGNOSTICS_PARENT=$(mktemp -d "${TMPDIR:-/tmp}/solid-native-ios-linking-test.XXXXXX")
...
if grep -R -F -q "SOLID_NATIVE_LINKING_FAILED" "$DIAGNOSTICS_DIRECTORY"; then
  echo "The iOS Linking proof emitted its JavaScript failure marker." >&2
  exit 1
fi

for marker in \
  SOLID_NATIVE_LINKING_READY \
  SOLID_NATIVE_LINKING_CAPABILITY_SUCCEEDED \
  SOLID_NATIVE_LINKING_OPEN_SUCCEEDED \
  SOLID_NATIVE_LINKING_CAUSALITY_SUCCEEDED \
  SOLID_NATIVE_LINKING_SETTINGS_SUCCEEDED \
  SOLID_NATIVE_LINKING_TEARDOWN_SUCCEEDED
do
  if ! grep -R -F -q "$marker" "$DIAGNOSTICS_DIRECTORY"; then
    echo "Missing iOS Linking physical-device proof marker: $marker" >&2
    exit 1
  fi
done

echo "Verified direct Linking capability, self-URL iOS delivery, private causal Fabric commit, app-specific Settings handoff, return, and non-terminating teardown on $SOLID_NATIVE_IOS_DESTINATION."
